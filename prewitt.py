...
def apply_prewitt_filter():
    ...
    prewitt_img = (prewitt_img / prewitt_img.max() * 255).astype(np.uint8)

    prewitt_img = Image.fromarray(prewitt_img)
    prewitt_img.thumbnail((300, 300))
    prewitt_img = ImageTk.PhotoImage(prewitt_img)

    configure_label(lbl_transformed_image, prewitt_img)

def start():
    window = tk.Tk()
    window.title("Prewitt Filter")
